...
public class Update extends JFrame {
	 ...
	 * constructor
	 */
	public Update(SecretBoardVo vo) {
		this();
		// 전달받은 아이디 세팅
		...
		this.vo = vo;
		txtSubject.setText(vo.getSubject());
		lblSerial.setText(vo.getSerial());
		txtTags.setText(vo.getTags());
		
		if (vo.getIsPublic()) {
			ckIsPublic.setSelected(true);
		} else {
			ckIsPublic.setSelected(false);
		}
		
		mainTextArea.setText(vo.getContent());
	}
	...
	private JButton getBtnWrite() {
		if (btnWrite == null) {
			btnWrite = new JButton("수 정");
			btnWrite.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					
					if (JOptionPane.showConfirmDialog(Update.this, "정말로 수정하시겠습니까?") == 0) {
						...
						b = new SecretBoardDao().update(vo);
	...
	private JLabel getLblNewLabel_6() {
		if (lblNewLabel_6 == null) {
			lblNewLabel_6 = new JLabel("     글 수정하기");
		...
		if (btnCancel == null) {
			...
			btnCancel.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent ae) {
					if (JOptionPane.showConfirmDialog(Update.this, "정말 취소하시겠습니까?") == 0) {
	...
	private JLabel getLblSerial() {
		if (lblSerial == null) {
			...
			lblSerial.setBounds(42, 19, 57, 15);
		}
		return lblSerial;
	}
		...
		if (button == null) {
			...
			button.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					if (JOptionPane.showConfirmDialog(Update.this, "정말로 삭제하시겠습니까?") == 0) {
						
						// 글의 PK 인 serial 만 넘겨준다
